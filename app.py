...
with st.form(key='periodontitis'):
    ...
    Q4 = st.number_input('Loose teeth without injury', key='p_Q4', key='p_Q2', min_value=0, max_value=120,  step=1, format="%d")   
...
with st.form(key='severeperiodontitis'):
    ...
    smoke = st.number_input('Smoking', key='s_smoke', key='p_Q2', min_value=0, max_value=5,  step=1, format="%d")  
    Q1 = st.number_input('Gum disease', key='s_Q1', key='p_Q2', min_value=0, max_value=5,  step=1, format="%d")
    Q2 = st.number_input('Health of teeth and gums', key='s_Q2', key='p_Q2', min_value=0, max_value=120,  step=1, format="%d")   
    Q4 = st.number_input('Loose teeth without injury', key='s_Q4', key='p_Q2', min_value=0, max_value=120,  step=1, format="%d")
    Q6 = st.number_input('Teeth do not look right', key='s_Q6', key='p_Q2', min_value=0, max_value=120,  step=1, format="%d")     
    st.form_submit_button('Calculate risk')
